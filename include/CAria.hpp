...
#define BY(X, Y) ((reinterpret_cast<Byte *>((&X)))[Y])
#define BRF(T, R) ((Byte)((T) >> (R)))
#define WO(X, Y) ((reinterpret_cast<Word *>((X)))[Y])
...
#define ReverseWord(W)                                                         \
  {                                                                            \
    (W) =                                                                      \
        (W) << 24 ^ (W) >> 24 ^ ((W)&0x0000ff00) << 8 ^ ((W)&0x00ff0000) >> 8; \
  }
#endif

/* Byte array를 Word에 싣는 함수.  _LITTLE_ENDIAN_의 경우
 * 엔디안 변환 과정을 거친다. */
...
#define WordLoad(ORIG, DEST)                                                 \
  {                                                                          \
    (DEST) =                                                                 \
        (0xff00ff00 & _lrotr((ORIG), 8)) ^ (0x00ff00ff & _lrotl((ORIG), 8)); \
  }
#endif
...
#define KXL                              \
  {                                      \
    t0 ^= WO(const_cast<Byte *>(rk), 0); \
    t1 ^= WO(const_cast<Byte *>(rk), 1); \
    t2 ^= WO(const_cast<Byte *>(rk), 2); \
    t3 ^= WO(const_cast<Byte *>(rk), 3); \
    rk += 16;                            \
  }
 ...
 * 또한 endian 변환과 가환이다.  또한, SBLi_M은 _LITTLE_ENDIAN_에서
 * 결과적으로 Word 단위로 endian을 뒤집은 결과를 준다.
 * 즉, FO, FE는 _BIG_ENDIAN_ 환경에서는 ARIA spec과 동일한 결과를,
 * _LITTLE_ENDIAN_ 환경에서는 ARIA spec에서 정의한 변환+endian 변환을
 * 준다. */
#define FO                                                  \
  {                                                         \
    SBL1_M(t0, t1, t2, t3)                                  \
...
#define GSRK(X, Y, n)                                                        \
  {                                                                          \
    ...
    r = (n) % 32;                                                            \
    WO(rk, 0) =                                                              \
        ((X)[0]) ^ (((Y)[(q) % 4]) >> r) ^ (((Y)[(q + 3) % 4]) << (32 - r)); \
    WO(rk, 1) =                                                              \
        ((X)[1]) ^ (((Y)[(q + 1) % 4]) >> r) ^ (((Y)[(q) % 4]) << (32 - r)); \
